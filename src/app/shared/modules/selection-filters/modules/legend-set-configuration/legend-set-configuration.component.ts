...
export class LegendSetConfigurationComponent implements OnInit, OnDestroy {
  ...
  legendSetLoadedState$: Observable<any>;

  constructor(private store: Store<LegendSetState>) {
    this.legendSetEntities$ = this.store.pipe(select(getAllLegendSets));
    this.legendSetLoadedState$ = this.store.pipe(select(getLegendSetLoaded));
  }
